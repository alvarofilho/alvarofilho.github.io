...
<style>
  ...
  .cmd-contact-section{padding-bottom:8rem}.cmd-contact-grid{display:grid;grid-template-columns:repeat(2,1fr);border-top:1px solid var(--cmd-line);border-left:1px solid var(--cmd-line)}.cmd-contact-grid>a,.cmd-contact-grid>div{display:grid;grid-template-columns:1fr auto;gap:1rem;min-height:145px;padding:1.2rem;color:var(--text);background:var(--cmd-panel);border-right:1px solid var(--cmd-line);border-bottom:1px solid var(--cmd-line);text-decoration:none}.cmd-contact-grid>a>span,.cmd-contact-grid>div>span{display:flex;align-items:center;gap:.45rem;color:var(--green);font-size:.56rem}.cmd-contact-grid strong{align-self:end;font-family:var(--sans);font-size:1.2rem;letter-spacing:-.03em}.cmd-contact-grid>a>:global(svg:last-child){grid-column:2;grid-row:1 / span 2}.cmd-contact-grid>a:hover{background:var(--card-h)}.cmd-contact-grid>a:hover strong{color:var(--green)}
